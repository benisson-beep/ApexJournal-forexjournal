...
export const TradeTable: React.FC<TradeTableProps> = ({ trades, onDeleteTrade }) => {
  ...
  const filteredTrades = trades.filter((trade) => {
    ...
    return matchesSearch && matchesSession && matchesDirection && matchesOutcome;
  });
  ...
  return (
    <div className="bg-[#0e131f] border border-[#1b2336] rounded-xl overflow-hidden shadow-xl">
      ...
      <div className="p-4 border-b border-[#1b2336] flex flex-col md:flex-row md:items-center justify-between gap-3 bg-[#0a0d14]/40">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-bold text-slate-100 uppercase tracking-wider flex items-center gap-2">
            <span>Trade Execution Log</span>
            <span className="text-xs font-mono font-normal text-slate-400 bg-[#1b2336] px-2 py-0.5 rounded-full">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="relative min-w-[200px]">
            ...
            <input
              ...
              className="w-full bg-[#131929] border border-[#1b2336] focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 rounded-lg pl-8 pr-3 py-1.5 text-xs text-slate-200 placeholder-slate-400 transition-all outline-none"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
          ...
          <select
            ...
            className="bg-[#131929] border border-[#1b2336] text-xs text-slate-300 rounded-lg px-2.5 py-1.5 outline-none cursor-pointer"
          >
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="border-b border-[#1b2336] bg-[#090d16] text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
              ...
              <th className="py-3 px-3 text-right">Pips</th>
              <th className="py-3 px-3 text-right">Net P&L</th>
              <th className="py-3 px-3 text-right">Realized R:R</th>
              <th className="py-3 px-3">Session</th>
              <th className="py-3 px-3">Tags & Setup</th>
            ...
            {filteredTrades.length === 0 ? (
              ...
              filteredTrades.map((trade) => {
                ...
                return (
                  <tr
                    key={trade.id}
                    className="hover:bg-[#131929]/70 transition-colors group"
                  >
                    {/* Date / Time */}
                    ...
                    {/* Session */}
                    <td className="py-3 px-3 whitespace-nowrap">
                      <span className="text-[11px] font-medium text-slate-300 bg-[#131929] border border-[#1b2336] px-2 py-0.5 rounded">
                        {trade.session}
                      </span>
                    </td>
